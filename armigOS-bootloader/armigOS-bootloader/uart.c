...
void uart_receive_string(char *buffer, int length)
{
	// read `length - 1` chars into `buffer`. will be 0-terminated
	int i;
	
	for (i = 0; i < length - 1; i++)
	{
		buffer[i] = uart_receive_blocking();
	}
	
	buffer[i] = '\0';
}

void uart_receive_typed_string(char *buffer, int length)
{
	int i;
	
	for (i = 0; i < length - 1; i++)
	{
		buffer[i] = uart_receive_echo();
	}
	
	buffer[i] = '\0';
}

void uart_read_line(char *buffer, int length)
{
	int i;
